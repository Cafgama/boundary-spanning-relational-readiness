% RUN_RESUMABLE_SELECTION_RULE_CONDITION
% Resumable selection-rule robustness experiment.
%
% This script runs one mechanism condition under one interaction-selection
% rule at a time.
%
% Main robustness comparison:
%   agent_first vs edge_uniform
%
% Current completed core simulations already use:
%   agent_first
%
% New runs needed:
%   edge_uniform versions of:
%     RB_low
%     BS_low
%     BS_high
...
% ------------------------------------------------------------
% User settings: choose ONE condition and ONE rule at a time
% ------------------------------------------------------------
% mechanism_condition = 'RB_low';
% mechanism_condition = 'BS_low';
 mechanism_condition = 'BS_high';

selection_rule = 'edge_uniform';
...
% Use the same seed bases as the mechanism-decomposition experiment
% so graph structures remain comparable to the original agent-first runs.
if strcmp(mechanism_condition, 'RB_low')
  seed_base = P.seed + 11000;
elseif strcmp(mechanism_condition, 'BS_low')
  seed_base = P.seed + 12000;
elseif strcmp(mechanism_condition, 'BS_high')
  seed_base = P.seed + 12000;
else
  error('Unknown mechanism condition.');
end

condition_label = [mechanism_condition, '_', selection_rule];

experiment_label = ['selection_', condition_label, '_NG', ...
...
if exist(checkpoint_file, 'file') == 2
  ...
  fprintf('\nNo checkpoint found. Starting new selection-rule condition.\n');
...
fprintf('\n============================================\n');
fprintf('RESUMABLE SELECTION-RULE CONDITION\n');
fprintf('Condition: %s\n', condition_label);
...
results.selection_rule = selection_rule;
...
fprintf('\n============================================\n');
fprintf('SELECTION-RULE CONDITION COMPLETED\n');
fprintf('Condition: %s\n', condition_label);
fprintf('Mechanism condition: %s\n', mechanism_condition);
fprintf('Selection rule: %s\n', selection_rule);
fprintf('Network architecture: %s\n', architecture);
