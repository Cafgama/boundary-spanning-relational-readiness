% RUN_RESUMABLE_TRANSLATION_CONDITION
% Resumable translation-capability robustness experiment.
%
% This script runs one boundary-spanning condition at a time while varying
% pi_BS, the success probability of boundary-spanning cross-boundary ties.
%
% Translation grid:
%   pi_BS = 0.55, 0.60, 0.65, 0.70
%
...
% Use the same seed base as BS_low and BS_high.
% This makes the boundary-spanning graph structures comparable.
seed_base = P.seed + 12000;

pi_label = sprintf('%03d', round(100 * pi_BS_value));

condition_label = ['TR_piBS_', pi_label];

experiment_label = ['translation_', condition_label, '_NG', ...
...
if exist(checkpoint_file, 'file') == 2
  ...
  fprintf('\nNo checkpoint found. Starting new translation condition.\n');
...
fprintf('\n============================================\n');
fprintf('RESUMABLE TRANSLATION-CAPABILITY CONDITION\n');
fprintf('Condition: %s\n', condition_label);
...
for g = start_graph:NG
  ...
  save(checkpoint_file, ...
    'condition_label', 'architecture', 'pi_BS_value', ...
    'NG', 'NT', 'P', 'seed_base', ...
...
results.pi_BS_value = pi_BS_value;
...
save(raw_file, ...
  ...
  'pi_BS_value', 'NG', 'NT', 'seed_base', 'elapsed_time');
...
fprintf('\n============================================\n');
fprintf('TRANSLATION CONDITION COMPLETED\n');
fprintf('Condition: %s\n', condition_label);
fprintf('Network architecture: %s\n', architecture);
fprintf('pi_out: %.2f | pi_BS: %.2f\n', P.pi_out, P.pi_BS);
fprintf('Runs: %d\n', results.n_runs);
