function F = fluid_learning_readiness_symmetric(Omega, p, x, w0, alpha, ell, Theta, C, max_windows)
% FLUID_LEARNING_READINESS_SYMMETRIC
% Coupled deterministic fluid-admission / exact-segment mean-learning model.
%
% Within each fluid active-set segment, actor i receives a productive learning
% update on an attempted interaction with probability p_i*A*ell_i, where A is
% the active responsibility mass of the opposite symmetric module. Therefore
% the residual first moment obeys
%
%   r_i(t+dt) = r_i(t) * (1-alpha*p_i*A*ell_i)^dt.
%
% Capacity resets each window; actor learning states persist.
  ...
  S = fluid_active_segments_symmetric(Omega, p, x);
  residual = 1-w0;
  t_total = 0;
  W = mean_W(p, residual);
  ...
  F.alpha = alpha;
  F.ell = ell;
  F.first_exhaustion_attempt = C*S.first_exhaustion_s;
  F.segment_template = S;
  ...
  for window = 1:max_windows
    ...
    for k = 1:S.n_segments
      dt = C*(S.s_end(k)-S.s_start(k));
      if dt <= tol
        continue;
      end

      active = S.active_masks{k};
      A = S.A(k);
      q = zeros(1,n);
      q(active) = p(active).*A.*ell(active);
      base = 1-alpha*q;
      assert(all(base >= -tol) && all(base <= 1+tol), ...
        'Mean-learning segment multiplier left [0,1].');
