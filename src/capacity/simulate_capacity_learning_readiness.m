function out = simulate_capacity_learning_readiness(pA, pB, xA, xB, w0, alpha, ellA, ellB, Theta, C, D, max_windows, demand_seed, learning_seed)
% SIMULATE_CAPACITY_LEARNING_READINESS
% Coupled finite-capacity + transferable-learning first-passage simulator.
%
% Model v0.7 semantics:
%   1. A capacity window contains exactly D attempted cross-boundary demands.
%   2. Each module receives total integer capacity C at the start of a window.
%   3. Actor capacities are allocated from target shares xA/xB and reset at
%      every window boundary.
%   4. Actor learning states wA/wB persist across window boundaries.
%   5. An attempted pair is served iff both endpoints have remaining capacity.
%   6. Served interactions consume one capacity unit from each endpoint.
%   7. Blocked interactions consume no capacity and produce no learning update.
%   8. For a served interaction, each endpoint independently produces useful
%      transferable learning with probability ell_i.
%   9. First passage occurs at
%         T = inf{t : min(WA(t),WB(t)) >= Theta},
%      where Wg = sum_i p_i w_i.
%
% The complete demand sequence and latent learning marks are generated once
% from explicit, isolated seeds. Learning marks attached to blocked attempts
% are ignored; blocked attempts therefore never trigger a learning event.
% This common-random-number construction permits paired no-capacity
% counterfactuals without changing the model semantics.
  ...
  out = initialize_output(C, D, max_windows, T_max, pA, pB, xA, xB, ...
    xA_realized, xB_realized, cA, cB, MA, MB, demand_seed, learning_seed, R);
